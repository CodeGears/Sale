...
{
	//NSLog(@"%@", [attributeDict objectForKey:@"msdata:rowOrder"], nil);	
	if (![attributeDict objectForKey:@"msdata:rowOrder"] == 0) {
		lastRowOrder = (NSInteger)[[attributeDict objectForKey:@"msdata:rowOrder"] intValue];
	}
	if (beginDataset == YES)
	{
		...
		if (self.DataSetName != nil && beginTable == YES)
		{
			...
			if([self.currentColumn isEqualToString:@""] || self.currentColumn == nil) //check if current column is empty because it should always be empty before assigning a new column, this is reset in the end element
			{
				...
				if ([self.dicCurrentRow  objectForKey:self.currentColumn])
				{
					...
					//NSLog(@"rowOrder:%d", lastRowOrder, nil); //[attributeDict objectForKey:@"msdata:rowOrder"], nil);
					self.currentRow  = [NSString stringWithFormat:@"%d", lastRowOrder, nil];
