...
+ (id)databaseWithPath:(NSString*)aPath {
    ...
    //NSString *writableDB = [documentsDir stringByAppendingPathComponent:@"test.sqlite"];
    success = [fileManager fileExistsAtPath:aPath];
    
    if(success) return [[[self alloc] initWithPath:aPath] autorelease];
// copy database to document directory if not exist
    NSLog(@"copy database");
    NSString *defaultPath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"MJLite.db"];
    success = [fileManager copyItemAtPath:defaultPath toPath:aPath error:&error];
    if(!success)
