...
@implementation VisitOrderViewController
...
- (IBAction) HitPopup:(id)sender{
    if (!popOverCtrl) {
        OrderAddProductPopOverViewCTRL* viewCtrl = [[OrderAddProductPopOverViewCTRL alloc] initWithNibName:@"OrderAddProductPopOverViewCTRL" bundle:nil];
...
- (void)viewDidLoad
{
    ...
    //Init data
    dicData = [[NSDictionary alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Order" ofType:@"plist"]];
    
    
    keys = [[NSArray alloc] initWithObjects:@"Order", @"Entry", nil];
}
    ...
    if ([indexPath section] == 0) {
        ...
        if ([[array objectAtIndex:[indexPath row]] isEqualToString:@"_Header"]) {
            ...
            if (cell == nil) {
                ...
                [[NSBundle mainBundle] loadNibNamed:@"TableCell_Order" owner:self options:nil];
                cell = cellHeader;
